...
import com.example.foody.ui.fragments.ingredient.IngredientFragment
...
class DetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val fragments = listOf(OverviewFragment(), IngredientFragment(), InstructionsFragment())
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.save_to_favorite_menu -> {
                if (!recipeSaved) saveToFavorites(item) else removeFromFavorites(item)
            }
        }
    ...
    private fun changeMenuItemColor(item: MenuItem, colorId: Int) {
        item.icon.setTint(ContextCompat.getColor(this, colorId))
    }
}